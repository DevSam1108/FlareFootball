...
| **Piece A eating real kicks (ISSUE-035)** | **🟠 OPEN — Piece A applied 2026-04-29 then found to eat real kicks** | **`_makeDecision` gates on instantaneous `_currentKickState == KickState.idle` and resets. Field log 2026-04-29 showed a real zone-6 kick suppressed: KickDetector transitioned `confirming → idle` one frame before ImpactDetector's lost-frame trigger fired (race with Phase 3 idle-edge recovery). Gate must be widened to track historical "kickConfirmedDuringTracking" flag. Widening designed, NOT applied. See ADR-086, ISSUE-035.** |
| **"Ball in position" audio cadence broken (ISSUE-036)** | **🟠 OPEN — root-caused 2026-04-29, fix designed not applied** | **`else` branch at `live_object_detection_screen.dart:970-972` resets `_lastBallInPositionAudio = null` on any `inPosition=false` frame, including single-frame YOLO misses. 10-second cadence almost never holds in practice; user heard double-fires within 4 s. Fix: delete the `else` AND add `_lastBallInPositionAudio = null` inside the filter OFF block at line ~1006 (only resets cadence on real kick attempts, not on transient flickers). See ISSUE-036.** |
