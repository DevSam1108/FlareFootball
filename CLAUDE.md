...
| **Mahalanobis rescue identity hijacking (ISSUE-026)** | **🟡 PARTIALLY FIXED (2026-04-15)** | **Bbox area ratio check (2.0/0.5) blocks size-mismatched rescues. Eliminates false positive hijacking (3.8x-9x jumps). BUT threshold too aggressive — blocks legitimate fast-kick tracking (ISSUE-029). Needs tuning: compare against last measured area or relax to 3.0-3.5. See ADR-071.** |
...
| **Bbox area ratio check too aggressive (ISSUE-029)** | **🔴 OPEN** | **2.0/0.5 threshold blocks legitimate Mahalanobis rescues during fast kicks. Kalman predicted area diverges during pure predictions. 2/5 kicks went silent. Fix: compare against last measured area or relax threshold.** |
