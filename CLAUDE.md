...
| ~~Diagnostic CSV logger~~ (UNPLUGGED 2026-04-30) | `lib/services/diagnostic_logger.dart` — superseded by `DiagLogFile`; `start()` call commented out at `live_object_detection_screen.dart:834`; pending full removal (see "Pending Code-Health Work") |
| **Diagnostic log wrapper (NEW 2026-04-30)** | `lib/utils/diag_log.dart` — `diagLog(String msg)` helper that prepends `[HH:MM:SS.mmm]`. Used by every single-line `DIAG-*` print across the app |
| **On-device session log file (NEW 2026-04-30)** | `lib/services/diag_log_file.dart` — `DiagLogFile` singleton: in-memory buffer + 500 ms flush + `start()`/`stop()`/`append()`. Files at `Documents/diag_<YYYY-MM-DD>_<HH-MM-SS>.log`. Lifecycle bound to `LiveObjectDetectionScreen.initState`/`dispose`. Wired via Zone interceptor in `main.dart`. |
...
| Issue | Status | Notes |
|---|---|---|
| **~~Target circle false positives (ISSUE-022)~~** | **✅ RESOLVED by Phase 3 (2026-04-22), confirmed 2026-05-01** | **Anchor Rectangle Phase 3 spatial filter drops target-circle FPs at `_toDetections` before ByteTrack — confirmed in iPhone 12 field test on every frame. See ADR-077.** |
| **~~Zone accuracy: one-row-down bias (Bug 3) (ISSUE-034)~~** | **✅ RESOLVED by Path A (2026-04-28), confirmed 2026-05-01** | **Two firing mechanisms diagnosed (velocity-drop + state-flip lost-frame). Path A: disabled velocity-drop trigger (ADR-083) and made `_onBallMissing` also update zone/position/bbox state (ADR-084). State-flip kick (1→6→7, hit zone 7) field-validated post-fix on iPhone 12. Velocity-drop mechanism structurally eliminated by Change 2.** |
| **~~Premature announcements (ISSUE-034)~~** | **✅ RESOLVED by Path A (2026-04-28), confirmed 2026-05-01** | **Same bug as zone accuracy above; same fix. Velocity-drop trigger that fired at trackFrames=4–5 with `depthRatio≈0.45` is now disabled. Decisions fire only via edge-exit, lost-frame trigger, or maxTrackingDuration (3 s).** |
...
| **~~Mahalanobis rescue identity hijacking (ISSUE-026)~~** | **✅ RESOLVED by Phase 3 (2026-04-22), confirmed 2026-05-01** | **Two-layer fix: (1) ADR-072 bbox area ratio gate (2.0×/0.3× of last measured area) blocks the 3.8×–9× jumps during kick flight, (2) Phase 3 spatial filter drops decoy detections (target circles, posters, far-field UI) before they reach ByteTrack during waiting state. Hijack failure mode structurally eliminated. See ADR-072, ADR-077.** |
...
| **~~directZone unreliable for non-bottom zones~~** | **✅ RESOLVED by Path A (2026-04-28), confirmed 2026-05-01** | **Same fix as ISSUE-034. ADR-084: `_onBallMissing` now updates `_lastDirectZone` from Kalman-predicted positions. Pre-fix `_lastDirectZone` froze at the entry zone; post-fix it tracks the ball's actual progression through the grid. State-flip kick verified zone 7 announcement post-fix.** |
...
| **~~Session lock stuck ON (ISSUE-030)~~** | **✅ RESOLVED by Phase 3 (2026-04-22), confirmed 2026-05-01** | **Phase 3 added two recovery paths: (a) idle-edge `else if` at `live_object_detection_screen.dart:1013` — when a flickered/false kick returns to `KickState.idle` without firing a decision, `_anchorFilterActive` is re-armed and `_ballId.deactivateSessionLock()` is called; (b) 2 s safety timer at `:1008` — if no decision fires within 2 s of the OFF-trigger, `_onSafetyTimeout` clears the lock. Bounce-back balls also fail the Phase 3 spatial filter (their bbox center is outside `_anchorRectNorm`) so they don't engage KickDetector in the first place. Path A (2026-04-28) is a secondary safeguard. See ADR-077, ADR-078.** |
...
| **~~ImpactDetector trigger gap (ISSUE-038)~~** | **✅ CLOSED — not a real bug (2026-05-01)** | **Field log review (2026-05-01 sizeVelocity Phase 1 test) confirmed lost-frame trigger fires reliably at impact with correct `lastDirectZone`. The "continuously DETECTED through impact" scenario was a thought experiment, never observed in field tests post-Path-A. The two concrete failure cases originally cited (Zone-6 ISSUE-035, Zone-8 FP-stuck-tracker) collapse on inspection — they're separate bugs (Piece A timing race, BallIdentifier shape gate) that a positive impact trigger wouldn't fix. Phase 1 sizeVelocity exposure (4 lines) kept as diagnostic signal; Phase 2 (consume the signal) cancelled. See `memory-bank/issueLog.md` ISSUE-038 for full closing rationale.** |
| **Audio kick gate too narrow — rejects refractory** | **🟠 OPEN — fix designed not applied** | **Gate at `live_object_detection_screen.dart:1133–1134` accepts only `confirming || isKickActive`. Refractory rejected → reject branch calls `_impactDetector.forceReset()` → result wiped before UI rebuild. Real kicks landing decisions during refractory (FP-stuck-tracker scenarios — see ISSUE-037) are eaten. Phase 3 already prevents bounce-back, so the original justification for the narrow gate is partially redundant. One-line fix: add `\|\| state == KickState.refractory` to the accept condition. NOT applied.** |
...
| **Diagnostic logging infrastructure overhaul (ADR-088 / ADR-089 / ADR-090)** | **✅ APPLIED + iOS-VERIFIED 2026-04-30; Android pending** | **(1) New `diagLog()` wrapper at `lib/utils/diag_log.dart` prepends `[HH:MM:SS.mmm]` to every single-line `DIAG-*` print (28 occurrences migrated). (2) Multi-line blocks (CALIBRATION DIAGNOSTICS, IMPACT DECISION) carry `│ timestamp=...` as first inner line; PIPELINE START gets it transitively via embedded calibration block. (3) On-device per-session `.log` file via Zone interceptor in `main.dart` + `DiagLogFile` singleton — files at `Documents/diag_<YYYY-MM-DD>_<HH-MM-SS>.log`, lifecycle bound to `LiveObjectDetectionScreen.initState`/`dispose`, 500 ms flush. iOS `Info.plist` enabled `UIFileSharingEnabled` + `LSSupportsOpeningDocumentsInPlace` → file visible in iOS Files app + Finder. (4) `AUDIO-DIAG` renamed → `DIAG-AUDIO` for prefix uniformity. (5) `DiagnosticLogger` CSV unplugged via single-line comment-out; "Share Log CSV" button hides itself; full removal tracked in "Pending Code-Health Work". 177/177 tests passing. Field-verified end-to-end on iPhone 12.** |
...
**Validation rule for Path B (or any future refactor of `ImpactDetector`):** before merging, capture ≥5 monitor-video kicks with the existing `DIAG-IMPACT [DETECTED]` / `[MISSING]` per-frame trace + `IMPACT DECISION` block + `DIAG-AUDIO`. Compare pre/post traces frame-by-frame. Pass criterion: `_lastDirectZone` at decision time matches the visually observed impact zone for every kick, and the trigger fires after the ball has actually transited the grid (not at trackFrames=4–5). Same protocol that proved Path A.

### `lib/services/diagnostic_logger.dart` — full removal candidate (unplugged 2026-04-30)

The CSV diagnostic logger was unplugged on 2026-04-30 in favor of the new on-device per-session `.log` text file written by `DiagLogFile` ([lib/services/diag_log_file.dart](lib/services/diag_log_file.dart)). The unplug is a single commented-out `start()` call at [live_object_detection_screen.dart:834](lib/screens/live_object_detection/live_object_detection_screen.dart:834); without that call, every other `DiagnosticLogger` method becomes a no-op (internal `_active` guard stays false) and the "Share Log CSV" button hides itself (its `if (filePath != null)` condition fails). All CSV-related code is preserved as dead code.

**Pending cleanup (do as a single focused change):**
- Delete `lib/services/diagnostic_logger.dart`
- Remove all `DiagnosticLogger.instance.*` call sites and the `import` in [live_object_detection_screen.dart](lib/screens/live_object_detection/live_object_detection_screen.dart) (commented-out `start()`, dead `logFrame` / `logDecision` / `stop` calls, the "Share Log CSV" button block at line ~1293, the share-handler at line ~1693)
- Verify `share_plus` is no longer used anywhere else, then drop from `pubspec.yaml` if unused

The on-device `.log` file is now the single source of truth for diagnostic output. iOS retrieval works two ways via the `UIFileSharingEnabled` + `LSSupportsOpeningDocumentsInPlace` flags added to `ios/Runner/Info.plist` on 2026-04-30: (a) Files app on iPhone → On My iPhone → app folder, or (b) plug iPhone into Mac → Finder → device → Files tab → app folder.
